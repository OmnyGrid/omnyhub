import '../errors/hub_exception.dart';

/// Manual JSON read helpers shared by hand-written `toJson`/`fromJson` codecs.
///
/// OmnyHub follows the sibling packages' convention of hand-written codecs (no
/// code generation). These helpers centralise type-checking and error
/// reporting so each decoder stays terse and fails with a [ProtocolException]
/// (rather than a raw `TypeError`) on bad input.
class Json {
  const Json._();
